...
class InstanceConfig:
    ...
    instance_id: str
    port: int
    role: str = "worker"  # "controller" or "worker"
    parent_id: Optional[str] = None

class Memory:
    """Manages persistent memory between sessions"""
    ...
    def summarize_context(self, context: List[Dict]) -> Dict:
        """Summarize conversation context for storage"""
        if not context:
            return {}
        ...
        for msg in context:
            if msg["role"] == "user":
                summary["topics"].append(msg["content"])
            elif msg["role"] == "assistant" and isinstance(msg["content"], list):
                for content in msg["content"]:
                    if content.type == "text":
                        summary["important_points"].append(content.text)
                        
        if context:
            summary["last_exchange"] = {
                "user": next((msg["content"] for msg in reversed(context) 
                            if msg["role"] == "user"), None),
                "assistant": next((msg["content"] for msg in reversed(context)
...
class Assistant:
    ...
    - Interaction with the Anthropics API 
    - Handling user commands and tool execution
    - Inter-instance communication and task distribution
    ...
    def __init__(self, config: Optional[InstanceConfig] = None):
        ...
        saved_context = self.memory.retrieve("conversation_context")
        self.conversation_history = saved_context if saved_context else []
        ...
        # Track child instances
        self.child_instances: Dict[str, 'Assistant'] = {}

        self.tools = self._load_tools()

    def save_context(self):
        """Save current conversation context to persistent memory"""
        summary = self.memory.summarize_context(self.conversation_history)
        self.memory.store("conversation_context", self.conversation_history)
        self.memory.store("context_summary", summary)
        
    def reset(self):
        """Reset conversation while preserving memory"""
        # Save summary before clearing
        if self.conversation_history:
            summary = self.memory.summarize_context(self.conversation_history)
            self.memory.store("last_context_summary", summary)
            
        self.conversation_history = []
        self.total_tokens_used = 0
        
    async def shutdown(self):
        """Cleanup resources and save context"""
        # Save context before shutting down
        self.save_context()
        
        # Shutdown child instances
        for child_id, child in self.child_instances.items():
    ...
    async def async_chat(self, user_input: str) -> str:
        ...
        try:
            ...
            if self.thinking_enabled:
                async with self._thinking_spinner():
                    response = await self._async_get_completion()
            else:
                response = await self._async_get_completion()
                
            # Save context after successful exchange
            self.save_context()
