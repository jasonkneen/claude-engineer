...
class Memory:
    ...
    def _ensure_context_dirs(self):
        """Create the required directory structure for context storage"""
        os.makedirs('contexts/history', exist_ok=True) 
        os.makedirs('contexts/state', exist_ok=True)
    ...
    async def save_context(self, filename_prefix: str = "context"):
        ...
        Full history goes to contexts/history directory with timestamp,
        Summary goes to contexts/state/context_baton.json.
        ...
        try:
            ...
            if not self.summary:
                ...
                try:
                    ...
                    logging.error(f"Error updating summary before save: {str(e)}")
                    self.summary = ContextSummary(
                        key_points=[],
                        decisions=[],
                        important_context="Error occurred while generating summary", 
            ...
            # Set up file paths
            context_baton_file = 'contexts/state/context_baton.json'
            full_history_file = f"contexts/history/{filename_prefix}_{timestamp}.json"
    ...
    def load_context(self, load_full: bool = False) -> bool:
        ...
        try:
            ...
            if os.path.exists('contexts/state/context_baton.json'):
                try:
                    with open('contexts/state/context_baton.json', 'r') as f:
                        data = json.load(f)
            ...
            if load_full and last_session:
                context_file = f"contexts/history/context_{last_session}.json"
    ...
    def list_context_files(self) -> List[Dict[str, str]]:
        ...
        try:
            os.makedirs('contexts/history', exist_ok=True)
            files = [f for f in os.listdir('contexts/history') if f.startswith('context_') and f.endswith('.json')]
            return [{
                'timestamp': f.split('_')[1].split('.')[0],
                'filename': f,
                'path': os.path.join('contexts/history', f)
    ...
    def _find_latest_file(self) -> Optional[str]:
        """Find most recent context file in contexts/history directory.
        
        Returns:
            Optional[str]: Path to the most recent context file, or None if no context files found
        """
        try:
...
class Assistant:
    ...
    def _find_latest_file(self) -> Optional[str]:
        ...
        """
        try:
            context_files = [f for f in os.listdir('contexts/history') 
                        if f.startswith('context_') and f.endswith('.json')]
            if not context_files:
                return None
            return os.path.join('contexts/history', max(context_files, key=lambda x: os.path.getmtime(os.path.join('contexts/history', x))))
        except Exception as e:
