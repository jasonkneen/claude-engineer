...
class Assistant:
    ...
    async def _get_completion(self):
        ...
        Continuously processes tool usage until reaching a final response.
        """
        while True:  # Main loop to handle tool usage
            try:
                # Check cache first for this state of conversation
                ...
                # Update token usage based on response
                ...
                if response.stop_reason == "tool_use":
                    ...
                    if getattr(response, 'content', None) and isinstance(response.content, list):
                        # Store assistant response containing tool use
                        ...
                        # Continue loop to process next completion with tool results
                        continue
