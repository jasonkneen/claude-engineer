...
class Memory:
    ...
    def __init__(self):
        ...
        # Set max tokens before compression
        self.max_tokens_threshold = 6000
        self.recent_messages_to_keep = 10
    ...
    def add_exchange(self, role: str, content: Any):
        ...
        tokens = len(self.encoder.encode(content_str))
        ...
        if self.total_tokens >= self.max_tokens_threshold:
            self._compress_history()
        elif len(self.full_history) % 5 == 0:  # Update summary every 5 messages
            self._update_summary()

    def _compress_history(self):
        """Compress history when token count exceeds threshold"""
        if len(self.full_history) <= self.recent_messages_to_keep:
            return
            
        # Keep the most recent messages intact
        recent_messages = self.full_history[-self.recent_messages_to_keep:]
        older_messages = self.full_history[:-self.recent_messages_to_keep]
        ...
        for msg in older_messages:
            ...
            # Only include significant content in summary
            if any(marker in content.lower() for marker in ["important", "key", "critical", "decided", "planned"]):
                summary_points.append(content)
        
        # Create a compressed version of older messages
        summary_content = "Historical Context Summary: " + " ".join(summary_points[-5:])
        summary_tokens = len(self.encoder.encode(summary_content))
        ...
        # Update total tokens
        self.total_tokens = sum(msg.get("tokens", 0) for msg in self.full_history)
        self._update_summary()
