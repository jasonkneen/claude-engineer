...
class TestAgentTool(AgentBaseTool):
    ...
    def __init__(self, agent_id: str, role: Union[AgentRole, str], name: Optional[str] = None):
        """Initialize test agent with proper role handling"""
    ...
    async def _initialize_tests(self):
        """Initialize tests dictionary asynchronously"""
    ...
    async def _update_test(self, test_name: str, test_code: str) -> str:
        ...
        async with self._lock:
            if not test_name or test_name not in self.state.data.get('tests', {}):
